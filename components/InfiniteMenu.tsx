...
class InfiniteGridMenu {
  ...
  #render() {
    ...
    gl.clearColor(isDark ? 0 : 1, isDark ? 0 : 1, isDark ? 0 : 1, 0);
...
export default function InfiniteMenu({ items = [] }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <h2 className={`face-title ${isMoving ? 'inactive' : 'active'} text-slate-950 dark:text-white`}>{activeItem.title}</h2>

          <p className={`face-description ${isMoving ? 'inactive' : 'active'} text-slate-800 dark:text-slate-300`}> {activeItem.description}</p>

          {activeItem.techStack && (
            <div className={`tech-stack ${isMoving ? 'inactive' : 'active'}`}>
